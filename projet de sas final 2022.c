...
switch (choix) {
   ...
   case 1 : {
         ...
         printf("1-code\n");
         scanf("%d",&p[cmp].code);  //cmp compteur
         printf("2-nom\n");
         scanf("%s",p[cmp].nom);
         printf("3-quantite\n");
   ...
   case 2:{
	...
	goto Menu;
      ...
      case 5:{
        ...
        scanf("%d",&c); //recherche  produt a alementer first 
...
break; }
     ...
     int z;
      ...
      case 1 : {
                  ...
                  //abule
                   ...
                   for (i = 0; i <cmp+1 ; i++) {
                  //  for ( j = 0; j <=cmp-1; j++)
                        if (strcmp(p[j].nom, p[j+1].nom) == 1) { //p[j\>p[j+1]
                            swap= p[j];
                            p[j] = p[j+1];
                            p[j+1] = swap;}}
    ...
    goto Menu;
            ...
            case 2:{
                     ...
                     produit swap;
                     // 1 BOUCLE JAMAIS TWSL Lcase lakhra
                     for (i=0;i<cmp-1;i++){ // makiwsltch lcase lakhra
                    for (j=i+1;j<cmp;j++){ // kat9rn m3a case limn wraha
                ...
                printf ("le prix ttc de produit :%f\n", (p[ka].prix)*1.15);//prixttc =prix*15%;
}
   goto Menu;
break;}
  }
...
int c,q;
    ...
    scanf("%d",&c);
      printf("entrer la quantie de produit a achter \t ");
        ...
        for(i=0;i<cmp;i++){
          if(c==p[i].code ){
          	//enregestrer les donne de produit acheter in new struct statistique
          	p[i].quantite=p[i].quantite-q;
			   s[w].quantite=q;  //affecter q l s quantie
         s[w].code=p[i].code;
         s[w].prix=p[i].prix*1.15;
         strcpy(s[w].nom,p[i].nom);
         w++;
              
              somme=somme+(s[w].prix*q); //same somme
              total+=q;
